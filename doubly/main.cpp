...
int main(int argc, char** argv)
{
    int choice;
    string menu[8] = {
        "------------------CRUD OPERATIONS WITH DOUBLY LINKED LIST---------------------",
    ...
    */
        ...
        std::cout << "Constructing " << argc - 1 << " nodes with the provided arguments..." << std::endl;
           ...
           //traversing the arguments values and inserting nodes at tail of the list with nodes built with each item of argv
            for (int i = 1; i < argc; i++) {
                std::cout << argv[i] << " ==> ";
                DoublyLinkedList->TailInsert(argv[i]);
    ...
    while (true)
    {
        ...
        std::cout<<std::endl;
            ...
            {
                case 1:
                    DoublyLinkedList->HeadInsert(GetDataFromUser(false));
                    break;
                case 2:
                    int position;
                    std::cout << "Please enter the position where you want to insert data: ";
                    std::cin>>position;
                    DoublyLinkedList->PositionInsert(GetDataFromUser(false), position);
                    break;
                case 3:
                    DoublyLinkedList->TailInsert(GetDataFromUser(false));
                    break;
                case 4:
                    DoublyLinkedList->Search(GetDataFromUser(true));
                    break;
                case 5:
                    DoublyLinkedList->Delete(GetDataFromUser(true));
                    break;
                case 6:
