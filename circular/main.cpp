...
int main(int argc, char** argv)
{
    int choice;
    string menu[8] = {
        "------------------CRUD OPERATIONS WITH CIRCULAR LINKED LIST---------------------",
    ...
    */
        ...
        std::cout << "Constructing " << argc - 1 << " nodes with the provided arguments..." << std::endl;
           ...
           //traversing the arguments values and inserting nodes at tail of the list with nodes built with each item of argv
            for (int i = 1; i < argc; i++) {
                std::cout << argv[i] << " ==> ";
                CircularLinkedList->HeadInsert(argv[i]);
    ...
    while (true)
    {
        ...
        std::cout<<std::endl;
            ...
            {
                case 1:
                    CircularLinkedList->HeadInsert(GetDataFromUser(false));
                    break;
                case 2:
                    int position;
                    std::cout << "Please enter the position where you want to insert data: ";
                    std::cin>>position;
                    CircularLinkedList->PositionInsert(GetDataFromUser(false), position);
                    break;
                case 3:
                    CircularLinkedList->TailInsert(GetDataFromUser(false));
                    break;
                case 4:
                    CircularLinkedList->Search(GetDataFromUser(true));
                    break;
                case 5:
                    CircularLinkedList->Delete(GetDataFromUser(true));
                    break;
                case 6:
